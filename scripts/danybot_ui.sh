...
function install_danybot_ui() {
  if [[ -z $(moonraker_systemd) ]]; then
    local error="Moonraker not installed! It's recommended to install Moonraker first!"
    print_error "${error}"
    while true; do
      local yn
      read -p "${cyan}###### Proceed to install DanyBot UI without installing Moonraker? (y/N):${white} " yn
      case "${yn}" in
        Y|y|Yes|yes)
          select_msg "Yes"
          break;;
        N|n|No|no|"")
          select_msg "No"
          abort_msg "Exiting DanyBot UI setup ...\n"
          return;;
        *)
  ...
  status_msg "Initializing DanyBot UI installation ..."
  ...
  danybot_ui_port_check

  ### download danybot_ui
  download_danybot_ui

  ### ask user to install the recommended webinterface macros
  install_danybot_ui_macros

  ### create /etc/nginx/conf.d/upstreams.conf
  set_upstream_nginx_cfg
  ### create /etc/nginx/sites-available/<interface config>
  set_nginx_cfg "danybot_ui"
  ### nginx on ubuntu 21 and above needs special permissions to access the files
  set_nginx_permissions

  ### symlink nginx log
  symlink_webui_nginx_log "danybot_ui"

  ### add danybot_ui to the update manager in moonraker.conf
  patch_danybot_ui_update_manager

  fetch_webui_ports #WIP

  ### confirm message
  print_confirm "DanyBot UI has been set up!"
}

function install_danybot_ui_macros() {
  local yn
  while true; do
    echo
    top_border
...
function download_danybot_ui_macros() {
  ...
  patch_danybot_ui_config_update_manager

  ok_msg "Done!"
}

function download_danybot_ui() {
  local url
  url=$(get_danybot_ui_download_url)
  ...
  if wget "${url}"; then
    ...
    print_error "Downloading DanyBot UI from\n ${url}\n failed!"
    exit 1
  fi
}
...
function remove_danybot_ui_nginx_config() {
  if [[ -e "/etc/nginx/sites-available/danybot_ui" ]]; then
    status_msg "Removing DanyBot UI configuration for Nginx ..."
    sudo rm "/etc/nginx/sites-available/danybot_ui" && ok_msg "File removed!"
  fi
  if [[ -L "/etc/nginx/sites-enabled/danybot_ui" ]]; then
    status_msg "Removing DanyBot UI Symlink for Nginx ..."
    sudo rm "/etc/nginx/sites-enabled/danybot_ui" && ok_msg "File removed!"
  fi
}

function remove_danybot_ui_logs() {
  local files
  files=$(find /var/log/nginx -name "danybot_ui*" 2> /dev/null | sort)
...
function remove_danybot_ui_log_symlinks() {
  local files regex

  regex="${HOME//\//\\/}\/([A-Za-z0-9_]+)\/logs\/danybot_ui-.*"
...
function remove_legacy_danybot_ui_log_symlinks() {
  local files
  files=$(find "${HOME}/klipper_logs" -name "danybot_ui*" 2> /dev/null | sort)
...
function remove_danybot_ui_config() {
...
function remove_danybot_ui() {
  remove_danybot_ui_dir
  remove_danybot_ui_nginx_config
  remove_danybot_ui_logs
  remove_danybot_ui_log_symlinks
  remove_legacy_danybot_ui_log_symlinks

  ### remove danybot_ui_port from ~/.kiauh.ini
  sed -i "/^danybot_ui_port=/d" "${INI_FILE}"

  print_confirm "DanyBot UI successfully removed!"
}
...
function update_danybot_ui() {
  backup_before_update "danybot_ui"
  status_msg "Updating DanyBot UI ..."
  download_danybot_ui
  match_nginx_configs
  symlink_webui_nginx_log "danybot_ui"
  print_confirm "DanyBot UI successfully updated!"
}
...
function get_remote_danybot_ui_version() {
  [[ ! $(dpkg-query -f'${Status}' --show curl 2>/dev/null) = *\ installed ]] && return

  local tags
  tags=$(curl -s "https://api.github.com/repos/NotFabianML/danybot-ui/tags" | grep "name" | cut -d'"' -f4)
  echo "${tags}" | head -1
}

function compare_danybot_ui_versions() {
  local versions local_ver remote_ver
  local_ver="$(get_local_danybot_ui_version)"
  remote_ver="$(get_remote_danybot_ui_version)"

  if [[ ${local_ver} != "${remote_ver}" && ${local_ver} != "" ]]; then
    versions="${yellow}$(printf " %-14s" "${local_ver}")${white}"
    versions+="|${green}$(printf " %-13s" "${remote_ver}")${white}"
    # add moonraker to application_updates_available in kiauh.ini
    add_to_application_updates "danybot_ui"
  else
...
function get_danybot_ui_download_url() {
  local releases_by_tag tags tag unstable_url url

  ### latest stable download url
  url="https://github.com/NotFabianML/danybot-ui/releases/latest/download/danybot_ui.zip"

  read_kiauh_ini "${FUNCNAME[0]}"
  if [[ ${danybot_ui_install_unstable} == "true" ]]; then
    releases_by_tag="https://api.github.com/repos/NotFabianML/danybot-ui/tags"
...
function danybot_ui_port_check() {
...
function select_danybot_ui_port() {
  if [[ ${PORT_80_BLOCKED} == "true" ]]; then
    echo
    top_border
    echo -e "|                    ${red}!!!WARNING!!!${white}                      |"
    echo -e "| ${red}You need to choose a different port for DanyBot UI!${white}       |"
    ...
    while true; do
      ...
      if [[ ${new_port} =~ ${re} && ${new_port} != "${MAINSAIL_PORT}" ]]; then
        select_msg "Setting port ${new_port} for DanyBot UI!"
...
function patch_danybot_ui_update_manager() {
  ...
  for conf in ${moonraker_configs}; do
    if ! grep -Eq "^\[update_manager danybot_ui\]\s*$" "${conf}"; then
      ### add new line to conf if it doesn't end with one
      [[ $(tail -c1 "${conf}" | wc -l) -eq 0 ]] && echo "" >> "${conf}"

      ### add DanyBot UIs update manager section to moonraker.conf
      status_msg "Adding DanyBot UI to update manager in file:\n       ${conf}"
      /bin/sh -c "cat >> ${conf}" << MOONRAKER_CONF

[update_manager danybot_ui]
type: web
channel: stable
...
function patch_danybot_ui_config_update_manager() {
  local patched moonraker_configs regex
  regex="${HOME//\//\\/}\/([A-Za-z0-9_]+)\/config\/moonraker\.conf"
  moonraker_configs=$(find "${HOME}" -maxdepth 3 -type f -regextype posix-extended -regex "${regex}" | sort)
